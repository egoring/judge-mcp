...
**상황** — 교정·리라이팅 서비스를 운영하는 팀. 프롬프트나 모델을 바꿀 때마다 "품질이 좋아졌는지"를 눈으로 몇 개 찍어 보고 배포한다. 회귀는 배포 후 사용자 불만이 올라와야 알게 된다.

**도입 후** — 사람 라벨 골든셋이 시험지가 된다. 프롬프트 수정 → 골든셋 자동 채점 → 정확도가 기준선 아래면 머지 차단. 회귀를 배포 전에 잡는다.

CI 품질 게이트 예시 (GitHub Actions):

```yaml
- name: 골든셋 품질 게이트 — 정확도 0.8 미만이면 머지 차단
  run: |
    python scripts/run_golden.py --backend api > report.json
    python -c "import json,sys; sys.exit(0 if json.load(open('report.json'))['accuracy'] >= 0.8 else 1)"
```
